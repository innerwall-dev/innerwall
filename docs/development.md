...
- **Protobuf → Go.** Edit `proto/innerwall/v1/*.proto`, run `make proto`. Output lands in `internal/gen/innerwall/v1/`. `proto/buf.yaml` is the buf module root, so package `innerwall.v1` lives at `innerwall/v1/` and files import as `innerwall/v1/<name>.proto`. `proto/buf.gen.yaml` runs the plugins through `go tool`, so the output is pinned by `go.sum`.
...
- `build`, `test`, `lint` (Go + proto + Biome), `ui` (production build)
- `drift`: `scripts/check-drift.sh`, which runs `make proto sqlc` and fails on any diff or untracked generated file
- `proto-breaking`: `scripts/check-proto-breaking.sh`, which runs `buf breaking` against the PR's base and fails unless the PR title or body references an ADR (`ADR-NNNN`)
...
proto/innerwall/v1        the API contract
