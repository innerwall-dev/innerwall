...
`innerwall dev seed --estate` also loads the review estate on top of that fleet: the flow map's label groups (a few hundred workloads under nine `app` labels, thirty unlabeled ones), a second address group, two rulesets, and traffic with every decision, including agents that dropped flow records, so the map and the fleet can be reviewed at the shape they were designed for. `--estate-extra N` adds N more app groups of three workloads each, all reached from one group, to review the map at estate scale.

The flow map renders with `@xyflow/react` (ADR-0008) and lays out with `@dagrejs/dagre`; both load with the map's own chunk, so no other screen carries them. The map reads only the public surface: the source-by-destination rollup once per decision, every workload in the map's scope (a page at a time), and the address groups; a pair's windows come from `GET /flows` when the operator opens it. Nothing the graph library draws carries a color of its own: `src/index.css` points its custom properties at the tokens.

The review composites in `docs/img/console/` are regenerated by `ui/scripts/capture-composites.mjs`, which drives two running control planes (one seeded with the review estate, one on an empty database for the fresh-install states) through their consoles and puts each screen beside its design shot, or beside its other theme when the design has none. It needs Playwright and a Chromium it can launch, and the design package's shots directory; the header of the script has the invocation.
