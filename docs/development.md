...
- **Console.** `make console` builds `ui/dist/`, which `ui/embed.go` embeds into the control-plane binary; the operator listener serves it at every path outside `/api/v1` (ADR-0008, ADR-0021). `ui/dist/.gitkeep` is tracked (and copied back in from `ui/public/` on every build) so the embed directory is never empty on a fresh checkout; a binary built from a checkout where the console has not been built answers those paths with a problem document saying so. `ui/scripts/generate-api.mjs` runs before every console build and generates `ui/src/api/generated.ts` from `api/openapi.yaml` when that description is present; until it is, the hand-written types in `ui/src/api/types.ts` are the contract.
