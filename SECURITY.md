...
The full model lives in `ARCHITECTURE.md` §11 and ADR-0004/0011; the load-bearing properties:
...
- **Enrollment is token-gated** (scoped, expiring, usage-limited join tokens); agent identity is short-lived mTLS with control-plane-assigned SPIFFE-style IDs. There is no trust-on-first-use window.
