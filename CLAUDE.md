...
- **The agent contract is defined in `proto/` first**, and breaking proto changes fail CI without an ADR reference. The operator surface's REST endpoints are hand-shaped HTTP handlers in `internal/api` that hold no domain logic: every handler calls the same domain functions the command line calls, and a handler that grows logic of its own is the defect. (ADR-0007 as amended, ADR-0021)
...
- **The operator surface is one TLS-only listener beside the agent gateway, with one operator and binary authorization.** No plaintext listener option; no endpoint that sets or resets the password (the command line on the host does that); no role, scope, or permission field anywhere in the schema or the API; both credential forms (session cookie, bearer operator token) resolve to one principal and handlers never learn which. Every endpoint is a thin handler over the shared domain layer; TLS only; the login endpoint is the one named public route. (ADR-0021)
...
- Console: `make console` builds it into `ui/dist/` for the embed; `-tags noconsole` swaps in a stub so Go-only work never needs Node. Biome handles lint + format in `ui/` (one `biome.json`, no eslint/prettier); `ui/src/tokens.css` is the palette, verbatim from the design package, and colors are plain `var()` references to it.
