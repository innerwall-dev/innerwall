...
- **Statelessness is load-bearing:** no control-plane replica may hold state that matters beyond its process; anything durable goes in Postgres. The one exception is the signing authority's key, which is operator-provisioned configuration identical on every replica, never database state. (ADR-0019)
